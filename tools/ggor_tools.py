...
#%% The class defining the data object

class GGOR_data(object):
    """Define GGOR class based on shapefile dbase.

    """

    def __init__(self, dbfFile, BMIN=5., BMAX=1000., LAYCBD=0, w=1.0, nmax=None):
        """Initialize GGOR object.
        ...
        nmax: int or list of ints
            if int: maximum number of parcels to simulate
            if list of ints: indices of parcels to simulate
    ...
    def get_bofek_data(self, dbfFile, BOFEK='BOFEK'):
       ...
       excel: str
        ...
        """
        try:
            bofek = pd.read_excel(dbfFile + '.xls', sheet_name=BOFEK, index_col='BOFEK')
        except:
            bofek = pd.read_excel(dbfFile + '.xlsx', sheet_name=BOFEK, index_col='BOFEK')
    ...
    def set_RIV(self, gr, pe):
        """Return RIV dictionary for ditch in top layer.
    ...
    def set_GHB(self, gr, pe):
        """Return GHB dictionary for ditch in top layer.
    ...
    def set_seepage(self, gr, NPER):
        """Return seepage for GGOR model, ie in second aquifer.
    ...
    def set_ibound(self, gr):
        ...
        It uses column 'b' to limit the active cells.

        The 3D array filled with data stored in self.data according to what.
...
class Meteo_data(object):
    ...
    def __init__(self, meteoFile):
        self.data = pd.read_csv(meteoFile,
                                header=None,
                                parse_dates=True,
                                dayfirst=True,
    ...
    def testdata(self, p=0.01):
        """Turn self.data into a meteo test set.
    ...
    @property
    def summer(self):
        """Return logical array telling which dates are summer."""
        start =  3  # after March
        end   = 10  # till October
    ...
    @property
    def winter(self):
        """Return logical array telling which days are winter."""
        return NOT(self.summer)

    def len(self):
        """Return length of meteo time series."""
        return len(self.data)

# GXG this GXG stuff is too complicated, so make it a class


class Heads:
    """Define heads object."""

    def __init__(self, tindex, gr=None, fphds=None, ibound=None, hda=None):
        """Return heads object prepared from the modflow computed heads.
        ...
        tindex: pd.Datetime index
            times corresponding to the head data
        fphds: heads object
            simulated heads read in by flopy.
        ibound: npdarray of shape (ny, nx, 2)
            Modflow's boundary array. Modflow uses two layers
            in this context.
        hda:
            heads from analytical output
        """
        if len(tindex) != fphds.shape[0]:
            raise ValueError('len(tindex)= {} != len(hphds) = {}'.format(
                    len(tindex), fphds.shape[0]))

        self.tindex = tindex

        if fphds is not None: # then we have heads from modflow output
            if gr is None:
                raise ValueError('gr must not be none when fphds is not None.')
            if ibound is None:
                raise ValueError('ibound must not be noen when fphds is not None.')

            hds = fphds.get_alldata() # the flowpy way

            # Generate the array per parcel hds[t, 2, nrow] from
            #      hds [t, nlay, nrow, ncol]
            # take Area-weighted sum over the columns (axis = -1)
            hds = np.sum( hds[:, [0, 1], :, :] *
                ((ibound==1) * gr.Area[np.newaxis, :, :])[np.newaxis, :, :, :],
                axis=-1)
            # store heads with layer as first dimension
            self.hds = hds.transpose(1, 0, 2)

        else: # then we have heads from analytical output
            # hda must be of shape (t, 2, nrow)
            if hda is not None:
                raise ValueError('hdaß must not be None if fphds is None.')
            # store heads with layer as first dimension
            self.hds0 = hda.transpose(1, 0, 2)

        # get area-weighted average of the heads in each parcel


    def plot(self, selection=None, nmax=5, ilay=[0, 1],  **kwargs):
        """Plot heads time series for selection of parcels.
        ...
        """
        if not isinstance(ilay, [tuple, list]):
            ilay = list(ilay)

        selection = np.array(selection)
        if selection is None:
            selection = np.arange(min(nmax, self.hds0.shape[0]))
        else:
            selection = np.array(selection)
            if len(selection) > nmax:
                selection = selection[:nmax]
            assert np.all(selection >= 0) and np.all(selection < self.hds0.shape[0]),\
                "selection must be indices >0 and < {}".format(self.hds0.shape[0])
        ...
        for isel in selection:
            for il in ilay:
                ax.plot(day_ends, self.hds[il][isel], label="parcel {}".format(isel))
...
class GXG:
    ...
    def __init__(self, hds):
        ...
        Parameters
        ----------
            hds: hdsObj from class Heads (has hds.hds of shape(nt, 2, nrow))
        """
        self.tindex = hds.tindex
        self.hds = hds.hds0 # just the water table aquifer is involved

        # this data set may be quite large, i.e. shape(nt, nrow) O(millions)

        # Immediately compute the GLG, GHG and GVG dicts

        # This is a Series that linkes dates to the time index in hds
        # This allows us to pick rows of the head ndarray corresponding to
        # bimonthly measurement times
...
if __name__ == '__main__':
    ...
    GGpth = '/Users/theo/GRWMODELS/python/GGOR/'

    # get the spatial data
    dpth = os.path.join(GGpth, 'data/spatial/', proj)

    dbfFile = os.path.join(dpth, proj)
    ...
    # get the meteo data
    meteoFile = os.path.join(GGpth, 'data/meteo/', met)
