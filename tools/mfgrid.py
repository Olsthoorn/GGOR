...
class Grid:
    ...
    def lrc(self, x, y, z=None, Ilay=None):
        '''returns zero-based LRC indices (iL,iR, iC) of points
        points must be given as x, y, z or as x, y , iLay
        shape of x, y, z or iLay must be the same
        ...
        if Ilay is not None: Ilay = np.array(Ilay)
        ...
        if z is not None:
            Ilay = np.zeros_like(x, dtype=int)
            for i, (z, ix, iy) in enumerate(zip(z, Icol, Irow)):
                Ilay[i] = index(z, self.Z[:, iy, ix])
